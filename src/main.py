...
def getTimeForOperationsFile(thread_count, implementation_type, case, sample_size, mMember, mInsert, mDelete):
    ...
    command = "./main 1000 10000 " + str(implementation_type_int) + " " + str(thread_count) + " " + str(mMember) + " " + str(mInsert) + " " + str(mDelete) + " " + filename
    print(command)
...
def calculateAvgAndStd():
    ...
    for i in range(3):
        for j in range(3):
            if j == 0:
                filename = "results/Case" + str(i+1) + "/" + Implementaion(j+1).name + "_1.txt"
                print(filename)
                ...
                for k in range(4):
                    filename = "results/Case" + str(i+1) + "/" + Implementaion(j+1).name + "_" + str(2**k) + ".txt"
                    print(filename)
